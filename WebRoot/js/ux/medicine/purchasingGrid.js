...
var YDataGrid = function(config){
		...
		var Handler = {
						...
						print:function (callback){
							var posts= Grid.datagrid("getSelections");
							if(posts.length == 0){
							...
							for(var i=0;i<posts.length;i++){
								ids+=posts[i].preOrderId1+",";
							}
							$("#printPage").dialog("open").load(urls['msUrl'] +"PreOrderDetaiController/goPrintPage.do?ids="+ids);
						}
					};		
		//工具类
		var Utils = {
		...
		var bar_print = { 	id:'bar_print',
							text:'打印',
							iconCls:'icon-print',
							btnType:'print',
							handler:Events.print	
						};
		var toolbarConfig = [bar_add,bar_edit,bar_remove,bar_print];
		var getToolbar = function (){			
			...
			if (dataGrid.toolbar != undefined && dataGrid.toolbar.length > 0) {
				for ( var i = 0; i < dataGrid.toolbar.length; i++) {
					...
					if(bar.btnType=='print'){
						tbars.push({id:bar.id || bar_print.id,text:bar.text || bar_print.text ,iconCls: bar.iconCls || bar_print.iconCls,btnType: bar.btnType || bar_print.btnType,handler:bar.handler || bar_print.handler});
						continue;
					}
		...
		var initGrid = function(){
			...
			var dataconfig = {
				...
				toolbar:dataGrid.toolbar2 || getToolbar(),
